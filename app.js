...
function cardCode(paper) {
  if (paper.editorial?.cardCode) return paper.editorial.cardCode;
  const evidenceCode = {
    guideline: "DIR",
...
function normalizeCuratedCard(card, libraryOrder) {
  ...
  return enrichPaper({ ...createPaper({
    ...
    source: "Biblioteca curada DOD",
    evidenceType: card.evidenceType || "article",
    isOpenAccess: null,
    meshTerms: card.mesh?.term ? [card.mesh.term] : [],
    clinicalTopicIds: card.clinicalTopicIds || [],
    editorial: {
      displayTitle: card.title,
      displaySummary: card.summary,
      readerText: (card.body || []).join(" "),
      referenceText: card.citation || null,
      cardCode: card.mark || null,
      visual: { accent: card.accent || null, mark: card.mark || null },
    },
  }), domainOverride: "PubMed / MeSH", libraryCategory: card.category, libraryOrder, tags: card.tags || [] });
}
...
function renderCard(paper) {
  ...
  const translatedTitle = paper.editorial?.displayTitle || buildPtBrTitle(paper);
  const translatedAbstract = paper.editorial?.displaySummary || buildPtBrSummary(paper, 220);
...
function openReader(paper) {
  ...
  els.readerTitle.textContent = paper.editorial?.displayTitle || buildPtBrTitle(paper);
  els.readerOriginalTitle.textContent = `Original: ${paper.title}`;
  els.readerTags.innerHTML = "";
  [labelEvidence(paper.evidenceType), paper.origin === "curated" ? "Biblioteca DOD" : paper.source.includes("PubMed") ? "PubMed" : "OpenAlex", accessLabel(paper), "DOD"].forEach((tag) => {
    const item = document.createElement("span");
    item.className = "evidence-pill";
    item.textContent = tag;
    els.readerTags.append(item);
  });
  els.readerAbstract.textContent = paper.editorial?.readerText || paper.editorial?.displaySummary || buildPtBrSummary(paper, 900);
  els.readerSource.textContent = [paper.journal || paper.source, paper.year || "ano não informado"].filter(Boolean).join(" • ");
  els.readerTrace.textContent = trace.join(" • ") || "Metadado mínimo disponível.";
  els.readerAuthors.textContent = paperAuthorsText(paper) || paper.editorial?.referenceText || "Autores não identificados";
